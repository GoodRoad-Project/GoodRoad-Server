...
create table if not exists obstacle_review (
    ...
    taken_by_moderator_id  bigint references users(id) on delete set null,
...
create index if not exists ix_obstacle_review_status_created_asc
    on obstacle_review(status, created_at asc);
...
create index if not exists ix_obstacle_review_taken_by_moderator
    on obstacle_review(taken_by_moderator_id);

create table if not exists obstacle_review_photo (
    id         bigint generated by default as identity primary key,
    review_id  bigint not null references obstacle_review(id) on delete cascade,
